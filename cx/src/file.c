...
cx_file_t* cx_file_open(const char* _filePath, CX_FILE_OPEN_MODE _mode)
{
    ...
    switch (_mode)
    {
    case CX_FILE_OPEN_READ:    mode = "r"; break;
    case CX_FILE_OPEN_WRITE:   mode = "w"; break;
    case CX_FILE_OPEN_APPEND:  mode = "a"; break;
    default:
...
int32_t cx_file_line_read(cx_file_t* _file, uint32_t _number, char* _buffer, uint32_t _bufferSize)
{
    ...
    CX_CHECK(CX_FILE_OPEN_READ == _file->mode, "_file must be opened as FILE_OPEN_READ");
...
{
    CX_CHECK_NOT_NULL(_file);
    CX_CHECK(CX_FILE_OPEN_WRITE == _file->mode || CX_FILE_OPEN_APPEND == _file->mode,
...
void cx_file_lines_for_each(cx_file_t* _file, cx_file_for_each_cb _cb, void* _userData)
{
    ...
    while (1)
    {
        lineLength = cx_file_line_read(_file, lineNumber, buffer, bufferSize);
...
void cx_file_lines_map(cx_file_t** _file, cx_file_map_cb _cb, void* _userData)
{
    ...
    cx_file_t* output = cx_file_open(outputFilePath, CX_FILE_OPEN_WRITE);
    ...
    cx_file_delete(*_file);
    cx_file_close(output);
    
    rename(outputFilePath, inputFilePath);
    (*_file) = cx_file_open(inputFilePath, CX_FILE_OPEN_READ);
